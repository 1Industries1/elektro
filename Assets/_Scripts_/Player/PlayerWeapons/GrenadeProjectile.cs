...
public class GrenadeProjectile : NetworkBehaviour
{
    ...
    [Header("Post-Explode Despawn")]
    public float despawnDelayAfterExplode = 3f;
    private bool _pendingDespawn; // guard

    [Header("VFX/SFX")]
    public GameObject explosionVFXPrefab;
    public GameObject delayedVFXPrefab;
    ...
    private void ExplodeServer(Vector3 contactPoint)
    {
        ...
        // NICHT sofort despawnen → sondern verzögert
        ScheduleDespawnAfterExplodeServer(center);
        //NetworkObject.Despawn();
    }

    private void ScheduleDespawnAfterExplodeServer(Vector3 center)
    {
        if (!IsServer || _pendingDespawn) return;
        _pendingDespawn = true;
        StartCoroutine(DelayedVfxThenDespawnRoutine(center));
    }

    private IEnumerator DelayedVfxThenDespawnRoutine(Vector3 pos)
    {
        yield return new WaitForSeconds(despawnDelayAfterExplode);

        ApplyExplosionDamageServer(transform.position, _baseDamageFromShooter * 0.75f);

        // VFX nach 3s abspielen (ClientRpc)
        var posNow = transform.position;
        SpawnDelayedVfxClientRpc(posNow);
    ...
    private void ApplyExplosionDamageServer(Vector3 center, float damage)
    {
        if (!IsServer) return;
        ...
        {
            var enemy = c.GetComponentInParent<IEnemy>();
            if (enemy == null || alreadyDamaged.Contains(enemy)) continue;

            float dist = Vector3.Distance(center, c.ClosestPoint(center));
            float nd = Mathf.Clamp01(dist / radius);
            float dmg = damage * damageFalloff.Evaluate(nd);

            if (dmg > 0.05f && alreadyDamaged.Add(enemy))
                enemy.TakeDamage(dmg, _shooter, c.ClosestPoint(center));
        }
    }

    [ClientRpc]
    private void SpawnDelayedVfxClientRpc(Vector3 pos)
    {
        if (delayedVFXPrefab == null) return;

        var go = Instantiate(delayedVFXPrefab, pos, Quaternion.identity);

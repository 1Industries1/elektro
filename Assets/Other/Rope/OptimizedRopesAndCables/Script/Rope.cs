...
namespace GogoGaga.OptimizedRopesAndCables
{
    ...
    public class Rope : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            // Set EndPoint to Harpoon if it's not set
            if (endPoint == null)
            {
                SetEndPointToHarpoon();
            }
            ...
            if (endPoint != null && Vector3.Distance(endPoint.position, startPoint.position) < 0.1f)
            {
                lineRenderer.positionCount = 0;
            }
        }

        private void SetEndPointToHarpoon()
        {
            // Find the first object with the tag "Harpoon"
            GameObject harpoonObject = GameObject.FindGameObjectWithTag("Harpoon");

            // If a harpoon object is found, set it as the end point
            if (harpoonObject != null)
            {
                endPoint = harpoonObject.transform;
            }
        }
